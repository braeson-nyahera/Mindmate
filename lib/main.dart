...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        '/login': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: LoginScreen()))
            ]),
        '/signup': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: SignUpScreen()))
            ]),
        '/home': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(
                  child:
                      ConnectivityWrapper(child: MyHomePage(title: 'Mindmate')))
            ]),
        '/courses': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: CoursesList()))
            ]),
        '/forum': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: ForumsWidget()))
            ]),
        '/tutors': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: TutorsWidget()))
            ]),
        '/profile': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: ProfileWidget()))
            ]),
        '/notifications': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: NotificationsWidget()))
            ]),
        '/landing_page': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: LandingWidget()))
            ]),
        '/chats': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: ChatsWidget()))
            ]),
        '/message_details': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: MessageListScreen()))
            ]),
        '/tutor_details': (context) => Column(children: [
              if (offline) const OfflineBanner(),
              Expanded(child: ConnectivityWrapper(child: TutorDetails()))
            ]),
